...
var debug = require('debug')('xpl-culw:serial');
...
Culw.prototype.processSerialData = function(data) {
	...
	case "V":
		// Version
		debug("Version of CULW is " + params);
		this._waitVersion = false;
		return;
	}

	if (this._waitVersion) {
		debug("Waiting for CULW version ! Ignore command '" + data + "'");
		...
		this._serialWriter("\n\nV\n", function(error) {
			if (error) {
				debug("Request of Version throws error ", error);
			}
		});
	...
	if (fct) {
		try {
			...
			debug("Error while parsing " + params, x);
		}
	}

	debug("Unsupported command '" + data + "'");
};
...
Culw.prototype.processCommand_E = function(parameters) {
	...
	var sCumulatedValue = parameters.substring(8, 10) + parameters.substring(6, 8);
	var sCurrentValue = parameters.substring(12, 14) + parameters.substring(10, 12);
	var sMaximumValue = parameters.substring(16, 18) + parameters.substring(14, 16);
	...
	debug("EM: device  '" + device + "' cumulated=" + cumulatedValue + " value=" + currentValue + " maximum=" +
			maximumValue);
...
Culw.prototype.processCommand_T = function(parameters) {
	...
	if (parameters.length < 8) {
		debug("Invalid T format '" + parameters + "'");
		return;
	}
	...
	case 0x00:
		...
		case 0x2a:
		case 0x3a:
			debug("FHT: lime protection for houseCode '" + houseCode + "' val=" + val);
			return;
		case 0xaa:
		case 0xba:
			debug("FHT: lime protection for houseCode '" + houseCode + "' val=" + val);
		...
		case 0xa0:
		case 0xb0:
			debug("FHT: sync in the summer for houseCode '" + houseCode + "'");
			return;
		}

		switch (status & 0xf) {
		case 0:
			debug("FHT: Sync now " + val + " for houseCode '" + houseCode + "'");
			return;

		case 1:
			debug("FHT: Valve 100% for houseCode '" + houseCode + "'");
			this._xplWriter({
		...
		case 2:
			debug("FHT: Valve 0% for houseCode '" + houseCode + "'");
			this._xplWriter({
				device: "fht " + houseCode,
				type: "valve",
				current: 0,
		...
		case 6:
			...
			debug("FHT: Valve " + val + "% for houseCode '" + houseCode + "' (parameters=" + parameters + ")");
			this._xplWriter({
				device: "fht " + houseCode,
				type: "valve",
				current: val,
		...
		case 8:
			...
			debug("FHT: Offset of houseCode '" + houseCode + "' : " + val);
			return;
		}
	...
	case 0x01: // syn !
		debug("FHT: Housecode '" + houseCode + "' requets sync !");
		return;
	...
	case 0x3e:
		debug("FHT: Automatic mode of houseCode '" + houseCode + "' : " + ((val == 1) ? "Manual" : "Auto"));
		this._xplWriter({
			device: "fht " + houseCode,
			type: "mode",
	...
	case 0x41:
		val /= 2;
		debug("FHT: Desired temperature of houseCode '" + houseCode + "' : " + val + "°");
	...
	case 0x43:
		...
		val = (val * 256 + low) / 10;
		debug("FHT: Mesured temperature of houseCode '" + houseCode + "' : " + val + "°");
	...
	case 0x44:
		...
		debug("FHT: Warnings for houseCode '" + houseCode + "' : 0x" + val.toString(16) + " oldMask=0x" +
				mask.toString(16) + " diff=0x" + (mask ^ val).toString(16));
	...
	case 0x45:
		val /= 2;
		debug("FHT: Manual temperature of '" + houseCode + "' : " + val + "°");
	...
	case 0x82:
		val /= 2;
		debug("FHT: Day temperature of '" + houseCode + "' : " + val + "°");
		this._xplWriter({
			device: "fht " + houseCode,
			type: "comfortTemp",
			current: val,
	...
	case 0x84:
		val /= 2;
		debug("FHT: Night temperature of '" + houseCode + "' : " + val + "°");
		this._xplWriter({
			device: "fht " + houseCode,
			type: "economicTemp",
			current: val,
	...
	case 0x8a:
		val /= 2;
		debug("FHT: Window Open temperature of '" + houseCode + "' : " + val + "°");
		this._xplWriter({
			device: "fht " + houseCode,
			type: "windowOpenTemp",
			current: val,
	...
	debug("FHT: Unsupported command: houseCode='" + houseCode + "' command=0x" + code.toString(16) + " val=" + val +
...
Culw.prototype._xplProcessProgram = function(houseCode, index, parameter) {
	...
	debug("FHT: process program for houseCode '" + houseCode + "' index=" + index + " dayIndex=" + dayIndex +
			" periodIndex=" + periodIndex + " parameter=" + parameter);
	...
	debug("FHT: set index=" + periodIndex + " val=" + v + " (min)");
	return true;
};
...
Culw.prototype._updateProgram = function(houseCode) {
	...
	if (!ps) {
		debug("FHT: update program for houseCode '" + houseCode + "' : Nothing to update");
		return;
	}

	delete this._fhtPeriodsByHouseCode[houseCode];

	debug("FHT: update program for houseCode '" + houseCode + "' program=", ps);
	...
	debug("FHT: update program for houseCode '" + houseCode + "' => " + program);
...
Culw.prototype.processCommand_F = function(parameters) {
	...
	if (parameters.length < 8) {
		debug("FS20: Invalid format '" + parameters + "'");
		return;
	}
	...
	if (command <= 0x10) {
		...
		debug("FS20: Dim of '" + houseCode + "' device '" + device + "' : " + p + "%");
		this._xplWriter({
	...
	if (command <= 0x3a) {
		debug("FS20: ON of '" + houseCode + "' device '" + device + "'");
		this._xplWriter({
			device: "fs20 " + houseCode + "/" + device,
			type: "state",
			current: "on"
	...
	debug("FS20: Unsupported command : House code=" + houseCode + " device=" + device + " command=0x" +
			command.toString(16) + " extension=" + extension + "  parameters=" + parameters);
...
Culw.prototype.processXplMessage = function(message) {
	...
	if (message.bodyName != "control.basic") {
		debug("XPL: Unsupported bodyName=", message.bodyName, " body=", message.body);
		return;
	}
	...
	if (!command) {
		debug("XPL: Unknown command name  body=", message.body);
		return;
	}

	var device = body.device;
	if (!device) {
		debug("XPL: Unknown device body=", message.body);
		return;
	}
	...
	debug("XPL: Unknown device type=", message.body);
};
...
Culw.prototype._fhtCommand = function(body) {
	...
	if (fhtTempCommands[command]) {
		...
		if (current < 5.5 || current > 30.5) {
			debug("FHT-Command : Invalid current command body=", body);
			return false;
		}

		buf += fhtTempCommands[command] + origin + ((current < 10) ? "0" : "") + current + "\n";

		// TODO call T03 to get the size of remaining buffer

		debug("FHT-Command: set temperature '" + command + "' current='" + current + "' => " + buf);
	...
	case "mode":
		...
		debug("FHT-Command: set mode '" + command + "' current='" + current + "' => " + buf);
		this._serialWriter(buf.toUpperCase());
		break;
	}
