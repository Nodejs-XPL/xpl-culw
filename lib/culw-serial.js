...
Culw.prototype.processCommand_T = function(parameters) {
  ...
  case 0x00:
    ...
    case 6:
      ...
      debug("FHT: Valve", val, "% for houseCode=", houseCode, "(parameters=",
          parameters, ")");
      this._xplWriter({
        device : "fht " + houseCode,
        type : "valve",
  ...
  case 0x3e:
    debug("FHT: Automatic mode of houseCode=", houseCode, ":",
        ((val == 1) ? "Manual" : "Auto"));
    this._xplWriter({
      device : "fht " + houseCode,
      type : "mode",
  ...
  case 0x44:
    ...
    debug("FHT: Warnings for houseCode=", houseCode, ": 0x" + val.toString(16),
  ...
  case 0x45:
    val /= 2;
    debug("FHT: Manual temperature of houseCode=", houseCode, ":", val, "°");
  ...
  case 0x82:
    val /= 2;
    debug("FHT: Comfort temperature of houseCode=", houseCode, ":", val, "°");
    this._xplWriter({
      device : "fht " + houseCode,
      type : "comfortTemp",
      current : val,
  ...
  case 0x84:
    val /= 2;
    debug("FHT: Economic temperature of", houseCode, ":", val, "°");
    this._xplWriter({
      device : "fht " + houseCode,
      type : "economicTemp",
      current : val,
  ...
  case 0x8a:
    val /= 2;
    debug("FHT: Window Open temperature of houseCode=", houseCode, ":", val,
        "°");
    this._xplWriter({
      device : "fht " + houseCode,
      type : "windowOpenTemp",
  ...
  debug("FHT: Unsupported command: houseCode=", houseCode, "command=0x" +
...
Culw.prototype._xplProcessProgram = function(houseCode, index, parameter) {
  ...
  debug("FHT: process program for houseCode=", houseCode, " index=", index,
...
Culw.prototype._updateProgram = function(houseCode) {
  ...
  if (!ps) {
    debug("FHT: update program for houseCode=", houseCode,
        ": Nothing to update");
    return;
  }
...
Culw.prototype.processCommand_F = function(parameters) {
  ...
  if (command <= 0x10) {
    ...
    debug("FS20: Dim of houseCode=", houseCode, "device=", device, ":", p, "%");
    this._xplWriter({
  ...
  if (command <= 0x3a) {
    debug("FS20: ON of houseCode=", houseCode, "device=", device);
    this._xplWriter({
      device : "fs20 " + houseCode + "/" + device,
      type : "state",
      current : "on"
  ...
  debug("FS20: Unsupported command : House code=", houseCode, "device=",
...
Culw.prototype._fhtCommand = function(body) {
  ...
  if (fhtTempCommands[command]) {
    ...
    debug("FHT-Command: set temperature command=", command, "current=",
        current, "=>", buf);
