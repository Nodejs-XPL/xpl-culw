...
Culw.prototype.processSerialData = function(data) {
	...
	if (this._waitVersion) {
		console.log("Waiting for CULW version ! Ignore command '" + data + "'");
...
Culw.prototype.processCommand_E = function(parameters) {
	...
	console.log("EM: device  '" + device + "' cumulated=" + cumulatedValue + " value=" + currentValue + " maximum=" +
			maximumValue);
...
Culw.prototype.processCommand_T = function(parameters) {
	...
	case 0x00:
		...
		case 0x2a:
		case 0x3a:
			console.log("FHT: lime protection for houseCode '" + houseCode + "' val=" + val);
			return;
		case 0xaa:
		case 0xba:
			console.log("FHT: lime protection for houseCode '" + houseCode + "' val=" + val);
		...
		case 0xa0:
		case 0xb0:
			console.log("FHT: sync in the summer for houseCode '" + houseCode + "'");
			return;
		}

		switch (status & 0xf) {
		case 0:
			console.log("FHT: Sync now " + val + " for houseCode '" + houseCode + "'");
			return;
		...
		case 6:
			...
			console.log("FHT: Valve " + val + "% for houseCode '" + houseCode + "' (parameters=" + parameters + ")");
			this._xplWriter({
				device: "fht " + houseCode,
				type: "valve",
				current: val,
	...
	case 0x01: // syn !
		console.log("FHT: Housecode '" + houseCode + "' requets sync !");
		return;
	...
	case 0x3e:
		console.log("FHT: Automatic mode of houseCode '" + houseCode + "' : " + ((val == 1) ? "Manual" : "Auto"));
		this._xplWriter({
			device: "fht " + houseCode,
			type: "mode",
	...
	case 0x41:
		val /= 2;
		console.log("FHT: Desired temperature of houseCode '" + houseCode + "' : " + val + "°");
	...
	case 0x43:
		...
		val = (val * 256 + low) / 10;
		console.log("FHT: Mesured temperature of houseCode '" + houseCode + "' : " + val + "°");
	...
	case 0x44:
		...
		console.log("FHT: Warnings for houseCode '" + houseCode + "' : 0x" + val.toString(16) + " oldMask=0x" +
	...
	console.log("FHT: Unsupported command: houseCode='" + houseCode + "' command=0x" + code.toString(16) + " val=" + val +
...
Culw.prototype._xplProcessProgram = function(houseCode, index, parameter) {
	...
	console.log("FHT: process program for houseCode '" + houseCode + "' index=" + index + " dayIndex=" + dayIndex +
	...
	console.log("FHT: set index=" + periodIndex + " val=" + v + " (min)");
	return true;
};
...
Culw.prototype._updateProgram = function(houseCode) {
	...
	if (!ps) {
		console.log("FHT: update program for houseCode '" + houseCode + "' : Nothing to update");
		return;
	}

	delete this._fhtPeriodsByHouseCode[houseCode];

	console.log("FHT: update program for houseCode '" + houseCode + "' program=", ps);
	...
	console.log("FHT: update program for houseCode '" + houseCode + "' => " + program);
...
Culw.prototype.processCommand_F = function(parameters) {
	...
	if (command <= 0x10) {
		...
		console.log("FS20: Dim of '" + houseCode + "' device '" + device + "' : " + p + "%");
		this._xplWriter({
	...
	console.log("FS20: Unsupported command : House code=" + houseCode + " device=" + device + " command=0x" +
...
Culw.prototype.processXplMessage = function(message) {
	...
	if (message.bodyName != "control.basic") {
		console.log("XPL: Unsupported bodyName=", message.bodyName, " body=", message.body);
		return;
	}
...
Culw.prototype._fhtCommand = function(body) {
	...
	if (fhtTempCommands[command]) {
		...
		console.log("FHT-Command: set temperature '" + command + "' current='" + current + "' => " + buf);
	...
	case "mode":
		...
		console.log("FHT-Command: set mode '" + command + "' current='" + current + "' => " + buf);
		this._serialWriter(buf.toUpperCase());
		break;
	}
